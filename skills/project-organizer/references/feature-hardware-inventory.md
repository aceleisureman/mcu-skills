...
将功能清单、硬件清单、引脚表和使用说明合并到**同一个 Markdown 文件**。各部分内容要求与第 1~4 节相同，但章节标题统一升级一级（原 `#` 变为 `##`），顶部增加项目概述章节。
...
### 5.6 引脚冲突注意事项
列出同一引脚在不同示例/场景中的冲突提醒。
```

**单文件汇总要点**：
- 章节编号统一：概述=1，功能=2，硬件=3，引脚=4，使用说明=5
