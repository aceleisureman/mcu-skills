...
- 上拉电阻取值根据负载选择，常见范围 1kΩ~4.7kΩ，确保 DQ 上升沿无毛刺
- VDD 旁放 100nF 去耦电容
- 数据线暴露场景加 ESD 保护（PESD3V3L5UY）
- 总线长度建议 < 30cm
- 寄生供电模式：VDD 接 GND，需强上拉电路
- 防水探头型号内部已封装上拉电阻的，外部无需重复接
...
芯片内部时钟随温度和电压变化，必须严格遵循 1-Wire 标准时序：

**复位脉冲**：
- MCU 拉低总线 ≥ 480μs（芯片复位需 8 个内部时钟周期，每周期最大 60μs，即 8×60=480μs）
- 释放总线后等待 15μs 再检测存在脉冲
- 存在脉冲窗口 60~240μs，建议在释放后 60~70μs 处采样

**写一位（Write Bit）**：
- 下拉总线 > 1μs 后输出数据
- 信号须维持 ≥ 60μs（芯片在内部时钟周期结束时采样，需覆盖最大周期）
- 恢复时间 > 1μs
- 写 1：下拉 1~15μs 后释放
- 写 0：下拉 ≥ 60μs 后释放
...
**读一位（Read Bit）**：
- 下拉总线 > 1μs 后释放
- 采样必须在下拉开始后 < 15μs 内完成（芯片在一个内部时钟周期内输出，最小值 1μs）
- 采样后等待 ≥ 50μs 确保总周期 > 60μs
- 恢复时间 > 1μs

```
时序容差总结：

操作        最小下拉    最大采样    最小总周期    恢复
Reset       480μs       70μs处采样  960μs        —
Write 1     1μs         —           60μs         1μs
Write 0     60μs        —           60μs         1μs
Read        1μs         <15μs       60μs         1μs
```

**高低温环境稳定性建议**：
1. 初始化下拉时间延长 1 倍（确保 > 480μs）
2. 读写一位的总周期延长 1 倍（确保 > 60μs）
3. 如仍不稳定：缩短读位采样时间至原来一半（确保 < 15μs）
...
| 现象 | 可能原因 | 解决方案 |
|------|---------|----------|
| 读到全 0x00 | 硬件短路 / 传感器烧坏 | 检查 VDD 是否正常、DQ 是否与 GND 短路；测 DQ/VDD 到 GND 阻值判断是否烧坏 |
| 读到全 0xFF | 无存在脉冲 / 写指令未识别 / 读时序不规范 | 检查波形和连线；确认 DQ/VDD 电平；调整读写时序 |
| 读到乱码 | 读时序不规范 / MTP 上电 Load 出错 / DQ 毛刺 | 调整读时序；掉电重启；选择合适上拉电阻 (1k~4.7kΩ) |
| CRC 校验出错 | DQ 线毛刺 / CRC 计算方法有误 | GPIO 切换模式避免毛刺（STM32 由输出切上拉输入会产生 1~2μs 毛刺，改用浮空输入或直接操作寄存器）；核对 CRC-8/MAXIM 参数 |
| 温度读数不变 | 转换时间不足 / 两线模式无强上拉 / MTP Load 错误 | 等待合适转换时间；寄生供电模式转换时 DQ 需强上拉；掉电重启 |
| 读到 85°C | 转换未完成就读取（85°C = 上电默认值） | 确保 12bit 模式等待 750ms；或轮询 DQ 线变高后再读取 |
| 读到 -127°C | 数据线无上拉电阻 | 检查 4.7kΩ 上拉电阻焊接 |
...
### 性能指标
| 指标 | DS18B20 | 新一代 18B20 | SHT30 | NTC |
|------|---------|-------------|-------|-----|
| 单次读取耗时 | ~750ms (12bit) | ~12ms (快速模式) | ~20ms | <1ms |
| 最小采样间隔 | 750ms | 12ms (建议 500ms 兼容旧设备) | 1s(单次) | 10ms |
| 功耗 | 1.5mA(工作) | 1.5mA(工作) | 1.2mA(测量) | <0.1mA |

## 5. 常见问题与避坑指南

| 问题 | 原因 | 解决方案 |
|------|------|----------|
| DS18B20 读到 85°C | 转换未完成就读取（上电默认值） | 确保 750ms 或轮询 DQ 变高 |
| DS18B20 读到 -127°C | 数据线未接上拉电阻 | 检查 4.7kΩ 上拉电阻 |
| 1-Wire 时序异常 | 中断打断时序 | 关键时序段关中断 |
| DS18B20 高低温通信失败 | 内部时钟随温度漂移 | 初始化下拉和读写周期延长 1 倍；读采样缩短至 < 15μs |
| DS18B20 读到全 0x00 | 硬件短路或传感器烧坏 | 检查 VDD/DQ 对 GND 阻值 |
| DS18B20 读到全 0xFF | 无存在脉冲或时序不对 | 检查波形连线、调整读写时序 |
| DS18B20 CRC 出错 | DQ 毛刺或 GPIO 模式切换 | 改浮空输入避免毛刺；核对 CRC-8/MAXIM 参数 |
